...
def train_all_users(exp_train: Tuple[np.ndarray, np.ndarray, np.ndarray],
    ...
    """
    # 阶段1：初始化数据结构
    classifiers = {}  # 存储分类器的字典
    exp_y = exp_train[1]  # 获取用户标签数组
    users = np.unique(exp_y)  # 去重得到用户列表

    # 阶段2：从开发集采样负样本
    if num_forg_from_dev > 0:
        # 获取开发集中的随机伪造样本
        other_negatives = data.get_random_forgeries_from_dev(
            dev_set,
            num_forg_from_dev,
            rng
        )
    else:
        other_negatives = []  # 无开发集负样本时为空

    # 阶段3：逐用户训练分类器
    for user in tqdm(users):  # 使用进度条可视化
        # 创建当前用户的训练集（正样本 + 负样本）
        training_set = data.create_training_set_for_user(
            user,
            exp_train,
            num_forg_from_exp,
            other_negatives,
        ...
        # 训练分类器并存储
        classifiers[user] = train_wdclassifier_user(
            training_set,
            svm_type,
            C,
            gamma
